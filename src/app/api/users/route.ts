  ...
  try {
    const command = new ScanCommand({
      ...
      ProjectionExpression:
        "id, email, userName, firstName, lastName, userType, userStatus, lastLoginTime, createdAt, department, phoneNumber",
    });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (data.password) {
      const hashedPassword = await bcrypt.hash(data.password, 10);
      updateExpression +=
        ", #password = :password, #lastPasswordChange = :lastPasswordChange";
      expressionAttributeValues[":password"] = hashedPassword;
      expressionAttributeValues[":lastPasswordChange"] =
